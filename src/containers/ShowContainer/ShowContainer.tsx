...
const ShowContainer = () => {
    const {id} = useParams();
    const currentShow: Show | null = useAppSelector(selectCurrentShow);
    ...
    const getRender = (title: string, data: string) => {
        return (
    ...
    if (currentShow) {
        ...
        if (currentShow.rating.average) {
            rating = getRender('Rating', currentShow.rating.average.toString());
        }
        ...
        if (currentShow.officialSite) {
            website = (
                <p><a className="btn btn-outline-primary" target="_blank" href={currentShow.officialSite}>Official
                    Site</a></p>
            );
        }
